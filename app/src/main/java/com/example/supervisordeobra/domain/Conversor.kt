...
class Conversor(latitudeFinal: Double, longitudeFinal: Double) {
    ...
    fun converter(context: Context) {
        ...
        var segundaExcentricidade2 =
            primeiraExcentricidade * primeiraExcentricidade / (1 - primeiraExcentricidade * primeiraExcentricidade)
        ...
        var m1 =
            Math.toRadians(latitudeCoordenada) * (1 - primeiraExcentricidade2 / 4 - 3 * primeiraExcentricidade4 / 64 - 5 * primeiraExcentricidade6 / 256)
        var m2 =
            senoDuasAngulo * (3 * primeiraExcentricidade2 / 8 + 3 * primeiraExcentricidade4 / 32 + 45 * primeiraExcentricidade6 / 1024)
        var m3 =
            senoQuadroAngulo * (15 * primeiraExcentricidade4 / 256 + 45 * primeiraExcentricidade6 / 1024)
        ...
        val a = Math.toRadians(longitudeCoordenada - meridianocentral) * Math.cos(
            Math.toRadians(latitudeCoordenada)
        )
        var y = k * (m - m0 + n * Math.tan(Math.toRadians(latitudeCoordenada)) * ((Math.pow(
            a,
            2.0
        ) / 2 + ((5 - t + 9 * c + 4 * Math.pow(
            c,
            2.0
        )) * Math.pow(a, 4.0)) / 24 + (61 - 58 * t + Math.pow(
            t,
            2.0
        ) + 600 * c - 330 * segundaExcentricidade2) * Math.pow(
            a,
            6.0
        ) / 720)))

        var x = k * n * ((a + (1 - t + c) * Math.pow(a, 3.0) / 6) + ((5 - 18 * t + Math.pow(
            t,
            2.0
        ) + 72 * c - 58 * segundaExcentricidade2) * Math.pow(a, 5.0) / 120)) + fe
